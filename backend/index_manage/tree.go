...
type bPlusTree struct {
	bootUUID     utils.UUID  //固定的bootUUID用于指向rootId，其实存储的就是dataitem的id
	bootDataItem dm.DataItem //根节点dataitem
	bootLock     sync.Mutex  //根节点的锁
...
func Load(bootUUID utils.UUID, dm dm.DataManager) (BPlusTree, error) {
	// 根据bootUUID读取dataitem，其目的是为了获取根节点的id。
