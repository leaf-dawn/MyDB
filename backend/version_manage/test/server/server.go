...
func serve(conn net.Conn) {
	...
	for {
		...
		case "commit":
			...
			err := SM.Commit(tm.TransactionID(xid))
			if err != nil {
		...
		case "abort":
			xid, _ := utils.StrToUint64(cmds[1])
			SM.Abort(tm.TransactionID(xid))
			pkg = transporter.NewPackage([]byte("Abort"), nil)
		case "insert":
			xid, _ := utils.StrToUint64(cmds[1])
			uuid, err := SM.Insert(tm.TransactionID(xid), []byte(cmds[2]))
			if err != nil {
				pkg = transporter.NewPackage(nil, err)
			} else {
		...
		case "read":
			...
			result, ok, err := SM.Read(tm.TransactionID(xid), utils.UUID(uuid))
			if err != nil {
				pkg = transporter.NewPackage(nil, err)
			} else if ok == false {
		...
		case "delete":
			xid, _ := utils.StrToUint64(cmds[1])
			uuid, _ := utils.StrToUint64(cmds[2])
			ok, err := SM.Delete(tm.TransactionID(xid), utils.UUID(uuid))
			if err != nil {
				pkg = transporter.NewPackage(nil, err)
			} else if ok == false {
				pkg = transporter.NewPackage([]byte("nil"), nil)
